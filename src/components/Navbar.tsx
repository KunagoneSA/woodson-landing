...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled ? 'bg-accent/95 backdrop-blur-md shadow-lg shadow-accent/10' : 'bg-transparent'
    }`}>
        ...
        <div className="flex items-center justify-between h-20">
            ...
            <div className="relative" ref={langRef}>
              <button onClick={() => setLangOpen(!langOpen)}
                className={`flex items-center gap-1 px-3 py-1.5 text-[10px] font-heading font-600 tracking-[0.15em] transition-colors border ${
                  scrolled ? 'text-white border-white/50 hover:border-white' : 'text-white/80 border-white/30 hover:border-white/60'
                }`}>
                {languageLabels[lang]}
              ...
              {langOpen && (
                <div className={`absolute right-0 mt-2 border overflow-hidden min-w-[70px] ${
                  scrolled ? 'bg-dark border-white/10 shadow-lg' : 'bg-dark border-white/10'
                }`}>
                  {languages.map((l) => (
                    <button key={l} onClick={() => { setLanguage(l); setLangOpen(false) }}
                      className={`w-full px-4 py-2 text-[10px] font-heading font-600 tracking-wider text-left transition-colors ${
                        ...
                        scrolled ? 'text-white/60 hover:text-white hover:bg-white/10' : 'text-white/40 hover:text-accent hover:bg-white/5'
                      }`}>
                      {languageLabels[l]}
                    </button>
                  ))}
          ...
          <div className="md:hidden flex items-center gap-2">
            <div className="relative">
              <button onClick={() => setLangOpen(!langOpen)}
                className="flex items-center gap-1 px-2 py-1.5 text-[10px] font-heading font-600 tracking-[0.15em] text-white/80 border border-white/30">
                {languageLabels[lang]}
                <ChevronDown size={10} className={`transition-transform ${langOpen ? 'rotate-180' : ''}`} />
              </button>
              {langOpen && (
                <div className="absolute right-0 mt-1 border border-white/10 bg-dark/95 backdrop-blur-md overflow-hidden min-w-[60px] z-50">
                  {languages.map((l) => (
                    <button key={l} onClick={() => { setLanguage(l); setLangOpen(false) }}
                      className={`w-full px-3 py-2 text-[10px] font-heading font-600 tracking-wider text-left transition-colors ${
                        lang === l ? 'bg-accent text-white' : 'text-white/50 hover:text-white hover:bg-white/5'
                      }`}>
                      {languageLabels[l]}
                    </button>
                  ))}
      ...
      {mobileOpen && (
        <div className={`md:hidden border-t px-8 py-6 space-y-4 ${
          scrolled ? 'bg-accent/95 backdrop-blur-md border-white/10' : 'bg-dark/95 backdrop-blur-md border-white/5'
        }`}>
          {navLinks.map((l) => (
            <a key={l.key} href={l.href} onClick={() => setMobileOpen(false)}
              className={`block text-sm font-heading font-500 ${scrolled ? 'text-white/70 hover:text-white' : 'text-white/50 hover:text-accent'}`}>
              {t.nav[l.key]}
            </a>
          ))}
        </div>
